...
'''
This is the script for running all ab initio and functional annotation tools and updating the .gff files for all 50 genomes.

The input to this script includes:
 a) a directory that contains all the .faa files with known gene or RNA sequences from gene prediction
 b) a directory that contains all the .faa files with known protein sequences from gene prediction
 c) a directory that contains all the .fasta files from assembly genome

The output is a .gff file associated with each genome
Except for EGGNOG, other functional tools take less than 15 mins, so we are going to run all 50 genomes with each tool
For EGGNOG,we used CD-HIT to cluster the .faa files, and run EGGNOG on the centroid file. From eggnog result, we
...
Note: to run the script, you need to have these tools in the current working directory:
tmhmm-2.0c
pilercr1.06
signalp-5.0
ncbi-blast-2.8.1+
utilitiest folder containing the functional_annotation.py, toGFF3.py, clustering.py, mapping.py, mergeAll_to_gff.py, eggnog.py
operonDB.fasta
VFDBdb.fasta
../installed_tools/eggnog-mapper/emapper.py
the conda environment need to be activate in order to run install tools like card
'''
import argparse
import os
import subprocess
